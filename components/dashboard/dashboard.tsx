...
// ----------------------------------------------------
// 2. Nodes Tab Component
// ----------------------------------------------------
function DashboardNodes() {
  const nodes = useVotingStore((state) => state.nodes);

  return (
    <Card className="p-6 border-border/80 bg-surface/20 animate-fadeIn">
      <div className="mb-6">
...
// ----------------------------------------------------
// 3. Logs Tab Component
// ----------------------------------------------------
function DashboardLogs() {
  const auditLogs = useVotingStore((state) => state.auditLogs);
...
// ----------------------------------------------------
// 4. Parent LiveDashboard Component
// ----------------------------------------------------
export function LiveDashboard() {
  const [mounted, setMounted] = useState(false);
  const [activeTab, setActiveTab] = useState<"overview" | "nodes" | "logs">("overview");

  // Subscribe to stable/non-fluctuating selector values to avoid re-renders on node latency ticks
  const totalVotes = useVotingStore((state) => state.votes.YES + state.votes.NO + state.votes.ABSTAIN);
  const activeNodeCount = useVotingStore((state) => state.nodes.filter(n => n.status !== "offline").length);
  const totalNodeCount = useVotingStore((state) => state.nodes.length);
  const updateNodeLatency = useVotingStore((state) => state.updateNodeLatency);
      ...
      <div className="max-w-[1280px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <Card className="p-6 border-border/80 bg-surface/35 flex flex-col justify-between">
            ...
            <div>
              <h3 className="text-3xl font-extrabold text-foreground tracking-tight">
                {totalNodeCount > 0 ? ((activeNodeCount / totalNodeCount) * 100).toFixed(0) : "0"}%
              </h3>
              <p className="text-xs text-foreground/40 mt-1 font-semibold">
                {activeNodeCount} of {totalNodeCount} Nodes Online
              </p>
            </div>
          </Card>
        ...
        {activeTab === "nodes" && <DashboardNodes />}
        {activeTab === "logs" && <DashboardLogs />}

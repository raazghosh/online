...
function RegisterPageContent() {
  ...
  const { register, user, isInitialized, initializeSession } = useVotingStore();
  ...
  // Form Input States - Organization
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (accountType === "individual") {
      // Validate Full Name: Must contain both a first name and a last name
      const nameParts = fullName.trim().split(/\s+/);
      if (nameParts.length < 2 || !nameParts[0] || !nameParts[1]) {
        setSubmitError("Please enter your full name (both first and last name separated by a space)");
        return;
      }
      ...
      if (!mobileRegex.test(mobileNumber)) {
        setSubmitError("Mobile number must be a valid 10-digit number");
        return;
      }

      // Validate Voter ID
      if (!voterId.trim()) {
        setSubmitError("Voter ID is required");
        return;
      }

      // Validate Aadhaar ID (optional, but if provided must be exactly 12 digits)
      ...
      // Validate Organization Contact Number
      const mobileRegex = /^\d{10}$/;
      if (!mobileRegex.test(orgPhone)) {
        setSubmitError("Contact number must be a valid 10-digit number");
        return;
      }
    ...
    try {
      ...
      if (accountType === "individual") {
        ...
        // Cache details in localStorage for pre-population in login / onboarding
        localStorage.setItem(`username_${normalizedEmail}`, `${first_name} ${last_name}`.trim());
        localStorage.setItem(`phone_${normalizedEmail}`, mobileNumber);
        if (aadhaar) {
          localStorage.setItem(`aadhaar_${normalizedEmail}`, aadhaar);
        }
        localStorage.setItem(`voter_id_${normalizedEmail}`, voterId);
      } else {
        ...
        // Cache details in localStorage for pre-population in login / onboarding
        localStorage.setItem(`username_${normalizedEmail}`, orgName.trim());
        localStorage.setItem(`phone_${normalizedEmail}`, orgPhone);
        localStorage.setItem(`org_reg_no_${normalizedEmail}`, orgRegNo.trim());
        localStorage.setItem(`org_type_${normalizedEmail}`, orgType);
      }
          ...
          <div className="w-full max-w-xl bg-white/5 backdrop-blur-[20px] rounded-[24px] border border-white/8 hover:border-white/12 shadow-[0_20px_50px_rgba(0,0,0,0.3)] relative overflow-hidden p-6 sm:p-8 md:p-10 group transition-all duration-300">
            ...
            <div className="grid grid-cols-2 p-1 rounded-lg bg-white/[0.02] border border-white/5 relative z-10 mb-8 max-w-xs mx-auto">
              <button
                ...
                className={`relative py-2 rounded-md text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${accountType === "individual" ? "text-white" : "text-foreground/40 hover:text-foreground/70"
                  }`}
              >
              ...
              <button
                ...
                className={`relative py-2 rounded-md text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${accountType === "organization" ? "text-white" : "text-foreground/40 hover:text-foreground/70"
                  }`}
              >
                ...
                {accountType === "individual" ? (
                  ...
                  >
                    <div className="space-y-1">
                      <h3 className="text-2xl font-bold tracking-tight text-foreground">Create Individual Account</h3>
                      <p className="text-sm text-foreground/50">Join secure digital elections</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                      <div className="relative group">
                        ...
                        <input
                          ...
                          className="w-full h-12 pl-11 pr-4 rounded-xl bg-white/[0.02] border border-white/10 text-foreground text-xs placeholder-foreground/30 focus:bg-white/[0.04] focus:border-primary focus:ring-1 focus:ring-primary/20 focus:outline-none transition-all"
                        />
                      </div>
                      ...
                      <div className="relative group">
                        ...
                        <input
                          ...
                          className="w-full h-12 pl-11 pr-10 rounded-xl bg-white/[0.02] border border-white/10 text-foreground text-xs placeholder-foreground/30 focus:bg-white/[0.04] focus:border-primary focus:ring-1 focus:ring-primary/20 focus:outline-none transition-all"
                        />
                        <button
                          type="button"
                          onClick={() => setShowPassword(!showPassword)}
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-foreground/40 hover:text-foreground/75"
                        >
                    ...
                    <label className="flex items-start gap-2.5 text-xs text-foreground/60 hover:text-foreground cursor-pointer select-none py-1">
                      <input
                        type="checkbox"
                        required
                        checked={agreeTerms}
                        onChange={(e) => setAgreeTerms(e.target.checked)}
                        className="mt-0.5 rounded border-white/10 bg-white/5 text-primary focus:ring-0 focus:ring-offset-0 cursor-pointer"
                      />
                      <span>
                        I agree to the <a href="#" className="text-primary hover:underline font-semibold">Terms & Conditions</a> and consent to cryptographic verification.
                      </span>
                    </label>
                  ...
                  >
                    <div className="space-y-1">
                      <h3 className="text-2xl font-bold tracking-tight text-foreground">Register Organization</h3>
                      <p className="text-sm text-foreground/50">Create and manage elections</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                      ...
                      <div className="relative group">
                        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-foreground/40 group-focus-within:text-primary pointer-events-none transition-colors">
                          <Building className="w-4 h-4" />
                        </div>
                        <select
                          value={orgType}
                    ...
                    <label className="flex items-start gap-2.5 text-xs text-foreground/60 hover:text-foreground cursor-pointer select-none py-1">
                      ...
                      />
                      <span>
                        I agree to the <a href="#" className="text-primary hover:underline font-semibold">Terms & Conditions</a> and consent to administrative cryptographic validation.
                      </span>
                    </label>
